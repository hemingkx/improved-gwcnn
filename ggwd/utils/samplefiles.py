...
class SampleFile:
    ...
    def as_dataframe(self,
        ...
        if injection_parameters:
            ...
            # when you set mass to static arguments, you should change 'mass1' into a variable arguments like spin1z
            # when you get no variable arguments, please comment this if-else structrue only leave those in if
            '''
            ...
            for i in range(len(df_injection_samples)):
                _ = {k: v[i] for k, v in iteritems(self.data['injection_parameters'])}
                injection_params.append(_)
            df_injection_params = pd.DataFrame().append(injection_params,
                                                            ignore_index=True,
                                                            sort=True)
